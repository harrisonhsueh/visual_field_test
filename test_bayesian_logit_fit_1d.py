...
def run_bayesian_estimation(k=2, b=29, k_guess=2, noise_level=0.01, max_prob_guess=0.95, min_prob_guess=0.05, max_prob=0.95, min_prob=0.05):
    ...
    b_values = np.linspace(0, 50, 820)  # Example range for b
    ...
    for i in range(num_trials):
        ...
        next_intensity_refined = choose_next_intensity_1d(prior, b_values, max_prob_guess=0.95, min_prob_guess=0.05)
        ...
        print(f"Trial {i + 1}: Test intensity = {next_intensity:.4f}, Result = {result}, Intensity index = {next_intensity_index}, Ideal test intensity = {next_intensity_refined}")
        ...
        lower_bound, upper_bound = compute_confidence_window(prior, b_values, confidence_level=0.90)
